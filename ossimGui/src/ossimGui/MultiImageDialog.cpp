...
ossimGui::MultiImageDialog::MultiImageDialog(QWidget* parent, DataManager::NodeListType& nodeList)
: QDialog(parent),
  m_currentIdCounter(0)
{
   initDialog();
   initContent(nodeList);
}
...
void ossimGui::MultiImageDialog::initDialog()
{
   ...
   connect(m_imageTable->verticalHeader(), SIGNAL(customContextMenuRequested(QPoint)), this, SLOT(displayImageContextMenu(QPoint)));
...
void ossimGui::MultiImageDialog::initContent(DataManager::NodeListType& nodeList)
{
   ...
   for(ossim_uint32 idx = 0; idx < m_overlaysReg.size(); ++idx)
   {
      ...
      tlist.push_back(input->getImageGeometry()->getProjection()->getShortName());
...
void ossimGui::MultiImageDialog::displayImageContextMenu(QPoint pos)
{
